...
public class ActivityOtaUpgrade extends AppCompatActivity implements LightingService.IServiceCallback{
    ...
    private static final int DFU_STATE_INITIATING           = 1;       // Initiator -> Distributor
...
//                                    return;
    ...
    @Override
    public void onDfuStatus(byte status, int currBlkNo, int totalBlocksNum) {
        ...
        if(totalBlocksNum == 0)
            ...
            percent = (currBlkNo*100/totalBlocksNum);
        txt = "\nDFU progress : "+percent+"%";
