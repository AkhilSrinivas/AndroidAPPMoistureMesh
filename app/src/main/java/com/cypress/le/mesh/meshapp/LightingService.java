...
*/
    ...
    public Boolean changeNetwork(String newNetwork) {
        ...
        currentNetwork = newNetwork;
        Log.d(TAG, "changeNetwork = " + currentNetwork);
    ...
    public void onHslValueChange(String deviceName, int hue, int saturation, int lightness) {
        ...
        mMesh.hslSet(deviceName, (short)lightness, (short)hue, (short)saturation, true, 0, (short) 0);
    }

    public void onOnoffSet(String deviceName, boolean brightness) {
        Log.d(TAG, "OnLightBrightinessChange devicename =" + deviceName);
        if(brightness)
            mMesh.onoffSet(deviceName, true, true, 0, (short) 0);
        else
            mMesh.onoffSet(deviceName, false, true, 0, (short) 0);
    }
...
//        }
    ...
    @Override
    public void onDfuStatus(byte status, int currBlkNo, int totalblock) {
        mCallback.onDfuStatus(status, currBlkNo, totalblock);
    }

    @Override
    public void onSensorStatusCb(String componentName, int propertyId, byte[] data) {
        Log.d(TAG, "onSensorStatusCb LightingService");
    ...
    public interface IServiceCallback {
        ...
        void onDfuStatus(byte status, int currBlkNo, int totalBlocksNum);
